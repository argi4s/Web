...
function pricesUpload(input) {
    try {
        fileRead(input, function(data) {
            const productIds = data.data.map(prc => prc.id);
            const idsString = productIds.join(',');
            $.getJSON(`./php/products.php?ids=${idsString}`, function(products) {
                const upload = [];

                data.data.forEach(function(prc) {
                    products.forEach(function(product) {
                        console.log(`Comparing: "${product.product_name}" with "${prc.name}"`);
                        if (product.product_name === prc.name) {
                            prc.prices.forEach(function(price) {
                                upload.push({
                                    id: prc.id,
                                    date: price.date,
                                    price: price.price
