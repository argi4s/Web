window.onload = function () {
  ...
  function offerPins(res) { 
    res.forEach((store, index) => { // kanei epanalhpsh sto data kai gia kathe store krataei ta stoixeia kai to index (thesh)
      markerdata.push(index); // vazei ston markerdata th thesh 
      const storeLatLng = L.latLng(store.lat, store.lon); // apo8hkevei tis suntetagmenes tou kathe magaziou
      let distance = userLatLng.distanceTo(storeLatLng); // orizei mia metavhlhth me onoma distance kai se auth vazei thn apostash tou xrhth apo to kathe magazi 
      // Create a container for the marker popup content
      const container = $("<div />");
      markers[index] = L.marker([store.lat, store.lon], { icon: offerIcon }); // gia to kathe magazi vazei ena eikonidio vasei twn sutentagmenwn
      if (distance <= proximityThreshold) {
        container.html(
          `<button class="btn showOffer">View Offers</button><br><br><button class="btn submitOffer">Add Offer</button>`
        );
      } else {
        container.html( // synarthsh jquery pou orizei ta periexomenou enos stoixeiou sthn HTML
      ...
      container.on("click", ".showOffer", function () {
        let params = new URLSearchParams(); // dhmiourgei ena neo stigmiotupo tou object aytou to opoio xeirizetai URLs san strings
        params.append("discount_id", store.discount_id); // pros8etei sthn metavlhth params to discount_id tou store me kleidi discount_id
  ...
  // Fetch stores with discounts to display a different icon
  let storesWithDiscount = {};
  const discountsAjax = $.ajax({
    url: "./php/storesWithOffers.php",
    method: "GET", 
    dataType: "json",
    success: function (data) {
  ...
  function storePois(result) {
    ...
    storeBtn.off("click").on("click", function() {
      ...
      if (storePin.val() === "") {
        ...
        //displayedMarkers.push(markers[markerdata.length]); // used to add the markers in order to clear them
        markers[markerdata.length].addTo(map); // adding the most recently created marker to the map.
        map.setView([selectedStore.lat, selectedStore.lon], 15);
        markerdata[markerdata.length] = markerdata.length; //keep track of the number of markers created to clear
      }
    });
